...
1. Class skeleton — `__init__` takes the token stream and sets a position cursor. Remind learner to import the nodes as they will be used by the parser. 
...
The `comparison()` method handles all six comparison operators. Guide the learner to define a class-level list `comparison_tokens` containing all six `TokenType` values, then check `token_peek().token_type in comparison_tokens` in the while loop. This keeps the method clean and puts the operator set in one place — the same pattern as `single_char_map` and `keyword_map` in the lexer.
