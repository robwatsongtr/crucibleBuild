...
Unlike Python, the C++ pass cannot be tested incrementally. `main.cpp` references all components and the compiler requires all of them to exist before `make` will succeed. Do not tell the learner to run `make` after completing individual components — the full pipeline must be in place first.
