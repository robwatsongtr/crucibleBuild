...
`NumberNode` and `IdentifierNode` are the leaves of the tree. They don't recurse. These are the base cases — every evaluation bottoms out here.

```
NumberNode:
    return the number value directly
...
### Expression nodes — recurse and combine

`BinaryOpNode` evaluates both children then combines their results:

```
BinaryOpNode:
    left  ← evaluate(left child)
    right ← evaluate(right child)
    look up the operator in the operator map
    return operator(left, right)
```

`UnaryOpNode` evaluates its single child:

```
UnaryOpNode:
    operand ← evaluate(operand child)
    look up the operator in the operator map
...
Statement nodes produce side effects rather than values.

```
AssignNode:
    value ← evaluate(expression child)
    store value in symbol table under var_name
...
### Control flow nodes — conditional execution

```
ConditionalNode:
    condition ← evaluate(condition child)
    if condition is truthy:
        evaluate the then_block
    else if an else_block exists:
        evaluate the else_block

WhileNode:
    loop:
        condition ← evaluate(condition child)
        if condition is falsy: stop
        evaluate the body_block
```

The while loop re-evaluates the condition on every iteration. The condition reads from the symbol table, which the body mutates. This is how loops terminate — the body eventually changes a variable that makes the condition false.

### Container nodes — iterate
