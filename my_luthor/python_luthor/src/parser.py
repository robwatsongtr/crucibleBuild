...
class Parser:
    ...
    def comparison(self):
        ...
        while self.token_peek() and self.token_peek().token_type in self.comparison_tokens:
            op = self.token_peek() # need to check what the token is first, peek at it)
            self.consume(op.token_type) # does validation, grabs, and advances
            r_node = self.term() # grab right side 
            root = BinaryOpNode(root, r_node, op)
